...
EdgesOrientation<-function (gInput,NQ=NQ,verbose = FALSE)
{
  ...
  g1=g
  p <- nrow(g)
#Tarmat (adjacency for directed graph) matrix updated in every steps, which used for the output of final topology
...
#Step-1
#Scenario-1: when we're working genetic variant is the first column
#Case-1: Relation between x (genetic variant) and other's nodes (genes)
         if(NQ>0)
          {
         for (iq in 1:NQ)
           {
         RWQ=which(g[iq, ] == 1) #Pullout relation between x (genetic variant) and other's nodes
         tarmat[iq, RWQ]  <- 1   #Direction will be genetic variant-->other's nodes (genes)
        if (length(RWQ)!=0)
          {
        for (qq in 1:length(RWQ))
          {
#Condition for the direction of genetic variant to genetic variant
       if (NQ>1 & RWQ[qq]<=NQ & tarmat[iq, RWQ[qq]] == 1)
       {
        tarmat[RWQ[qq], iq]  <- 1  #Direction for genetic variant to genetic variant will be genetic variant<-->genetic variant
       }
        }
         }
         }
...
#Start to orient v-structures
     ...
     for (i in seq_len(nrow(ind)))
    {
     ...
     for (z in allZ)
    {
#For each triple of vertices x, y, z such that the pair x, y and the pair z, y are each adjacent in the skeleton
#but x, z are not, is oriented based on the information saved in the conditioning sets of test
#More precisely, x-y-z is directed x-->y<--z if x and z conditionally dependent given y
...
#Start to orient remaining edges.
  repeat {
        ...
        {
          if(tarmat[S_gn,S11[d2]]==1||tarmat[S11[d2],S_gn]==1)  #ignore if already have direction
          {
            S111=integer(0)
          }
          else
